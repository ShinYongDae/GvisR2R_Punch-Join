...
IMPLEMENT_DYNCREATE(CRs232Smac, CObject)



CRs232Smac::CRs232Smac()
{
	m_hComDev = NULL;
...
//BEGIN_MESSAGE_MAP(CRs232Smac, CObject)
	//{{AFX_MSG_MAP(CRs232Smac)
...
/////////////////////////////////////////////////////////////////////////////
// CRs232Smac message handlers
//Rs232SmacWatchProc()
//통신을 하는 프로세저 즉 데이타가 들어왔을대 감시하는
//루틴 본루틴은 OpenComPort 함수 실행시 프로시저로 연결됨
//OpenComPort 함수 참조
DWORD Rs232SmacWatchProc(LPVOID lpData)
{
	DWORD       dwEvtMask ;
	OVERLAPPED  os ;
	CRs232Smac*      npRs232 = (CRs232Smac*) lpData ;
	char InData[MAXBLOCK + 1];
	int        nLength ;
	//idRs232Dev 라는 핸들에 아무런 com 포트가 안붙어 잇으면
	// 에라 리턴
	if ( npRs232 == NULL || !npRs232->IsKindOf( RUNTIME_CLASS( CRs232Smac ) ) )
		return (DWORD)(-1);
...
//데이타를 일고 데이타를 읽었다는
//메세지를 리턴한다.
void CRs232Smac::SetReadData(LPSTR data)
{
	...
	((CSmac*)m_pParent)->OnReceiveRs232(0,0);
}

//메세지를 전달할 hwnd설정
void CRs232Smac::SetHwnd(CWnd* pParent)
{
	m_pParent = pParent;
...
//컴포트를 설정한다.
void CRs232Smac::SetComPort(int port,DWORD rate,BYTE bytesize,BYTE stop,BYTE parity)
{
	m_nPort=port;
	m_dwBaudRate=rate;
...
//XonOff 즉 리턴값 더블 설정
void CRs232Smac::SetXonOff(BOOL chk)
{
	m_bXonXoff=chk;
}

void CRs232Smac::SetDtrRts(BYTE chk)
{
	m_nFlowCtrl=chk;
}

void CRs232Smac::ClearReceive()
{
	//memset(m_cRxBuffer, NULL, sizeof(BYTE)*MAXBLOCK );
	m_cRxBuffer = _T("");
}
...
BOOL CRs232Smac::CreateRs232Info()
{
...
BOOL CRs232Smac::OpenComPort()
{
	...
	if (bRetVal)//연결이 되었다면 fRetVal TRUE이므로
	{
		...
		AfxBeginThread((AFX_THREADPROC)Rs232SmacWatchProc,(LPVOID)this);
	}
	else
	{
		DestroyRs232();
...
BOOL CRs232Smac::SetupConnection()
{
	BOOL       bRetVal ;
//	BYTE       bSet ;
	DCB        dcb ;
...
//컴포트로 부터 데이타를 읽는다.
int CRs232Smac::ReadRs232Block(LPSTR lpszBlock, int nMaxLength )
{
	BOOL       bReadStat ;
	COMSTAT    ComStat ;
	DWORD      dwErrorFlags;
...
//연결을 닫는다.
BOOL CRs232Smac::CloseConnection()
{
	if(m_hComDev)
	{
...
BOOL CRs232Smac::WriteRs232Block( LPSTR lpByte , DWORD dwBytesToWrite)
{
