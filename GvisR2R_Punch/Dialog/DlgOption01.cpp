...
void CDlgOption01::OnBnClickedCheck13()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	BOOL bOn = ((CButton*)GetDlgItem(IDC_CHECK13))->GetCheck();
	pDoc->m_bUsePchFile = bOn;

	if (bOn)
	{
		::WritePrivateProfileString(_T("System"), _T("UsePchFile"), _T("1"), PATH_WORKING_INFO);
	}
	else
	{
		::WritePrivateProfileString(_T("System"), _T("UsePchFile"), _T("0"), PATH_WORKING_INFO);
	}
}


void CDlgOption01::OnBnClickedCheck14()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	BOOL bOn = ((CButton*)GetDlgItem(IDC_CHECK14))->GetCheck();
