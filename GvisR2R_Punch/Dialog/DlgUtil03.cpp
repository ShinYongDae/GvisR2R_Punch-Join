...
void CDlgUtil03::Disp(int nDir)
{
	...
	{
	case ROT_NONE:
		for(nC=0; nC<MAX_SPREAD_COL; nC++)			// nC, nR : Cam(or Reelmap) ����
		{
		...
		break;
	case ROT_CCW_90:
// 		for(int nC=0; nC<MAX_SPREAD_COL; nC++)			// nC, nR : Cam(or Reelmap) ����
// 		{
...
void CDlgUtil03::OnBnClickedBtnJudgeMk()
{
	...
	if (pDoc->WorkingInfo.LastJob.bUseJudgeMk)
	{
		...
		if (nCam < 0)
		{
			AfxMessageBox(_T("��/�� ��ŷ �� �ϳ��� �����ϼ���."));
			return;
		}

		if (IDNO == pView->MsgBox(_T("�̸�ŷ �׽�Ʈ�� �����Ͻðڽ��ϱ�?"), 0, MB_YESNO))
			return;
