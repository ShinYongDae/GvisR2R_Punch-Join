	...
	: CFormView(IDD_GVISR2R_PUNCH_FORM)
{
	...
	m_nSnTHREAD_UPDATAE_YIELD_ALL = 0;
	m_nSnTHREAD_UPDATAE_YIELD_INNER_ALL = 0;
...
UINT CGvisR2R_PunchView::ThreadProc3(LPVOID lpContext)	// UpdateYield()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[3].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pDoc->m_ListSerialYield.nTot > 0)
		{
			if (!pThread->m_bTHREAD_UPDATE_YIELD_UP && !pThread->m_bTHREAD_UPDATE_YIELD_DN && !pThread->m_bTHREAD_UPDATE_YIELD_ALLUP && !pThread->m_bTHREAD_UPDATE_YIELD_ALLDN)
			{
...
void CGvisR2R_PunchView::SwapUp(unsigned __int64 *num1, unsigned __int64 *num2) 	// 위치 바꾸는 함수
{
...
BOOL CGvisR2R_PunchView::SortingInUp(CString sPath, int nIndex)
{
	...
	else
	{
		...
		unsigned __int64 nYear = _tstoi(sYear);
		unsigned __int64 nMonth = _tstoi(sMonth);
		unsigned __int64 nDay = _tstoi(sDay);
		unsigned __int64 nHour = _tstoi(sHour);
		unsigned __int64 nMin = _tstoi(sMin);
		unsigned __int64 nSec = _tstoi(sSec);
...
void CGvisR2R_PunchView::SwapDn(unsigned __int64 *num1, unsigned __int64 *num2) 	// 위치 바꾸는 함수
{
	unsigned __int64 temp;
	...
	if (pDoc->m_bVsStatusUp)
	{
		...
		if (m_bPcrInShareVs[0])
		{
			if (pDoc->m_pFile)
			{
				...
				{
					m_nShareUpS = nSerial;
					sMsg.Format(_T("상면 : %d PCR 공유에서 버퍼로 위치 이동"), nSerial);
					pDoc->LogAuto(sMsg);
				}
				pDoc->m_pFile->DelPcr(sSrc, nSerial);
				m_bPcrInShare[0] = FALSE;
		...
		if (bDualTest)
		{
			...
			if (m_bPcrInShareVs[1])
			{
				if (pDoc->m_pFile)
				{
					...
					{
						m_nShareDnS = nSerial;
						sMsg.Format(_T("하면 : %d PCR 공유에서 버퍼로 위치 이동"), nSerial);
						pDoc->LogAuto(sMsg);
					}					
					pDoc->m_pFile->DelPcr(sSrc, nSerial);
					m_bPcrInShare[1] = FALSE;
...
BOOL CGvisR2R_PunchView::IsRun()
{
	if (m_sDispMain == _T("정 지"))
	{
		...
		if (pDoc->m_pMpeSignal[0] & (0x01 << 5) && pDoc->m_pMpeSignal[19] & (0x01 << 15))
		{
			...
			{
				if (pView->m_nMkStAuto > MK_ST + (Mk2PtIdx::DoneMk) && pView->m_nMkStAuto < MK_ST + (Mk2PtIdx::DoneMk) + 8)
					return TRUE; // Last Shot의 마지막 처리가 될 때까지 작업을 계속함.
			}
			else if (pDoc->WorkingInfo.LastJob.nAlignMethode == FOUR_POINT)
			{
				if (pView->m_nMkStAuto > MK_ST + (Mk4PtIdx::DoneMk) && pView->m_nMkStAuto < MK_ST + (Mk4PtIdx::DoneMk) + 8)
					return TRUE; // Last Shot의 마지막 처리가 될 때까지 작업을 계속함.
			}
		}
...
void CGvisR2R_PunchView::InitAuto(BOOL bInit)
{
	...
	m_nSerialTHREAD_UPDATAE_YIELD[0] = 0; // 좌측 마킹 중인 버퍼의 시리얼
	m_nSerialTHREAD_UPDATAE_YIELD[1] = 0; // 우측 마킹 중인 버퍼의 시리얼
	...
	m_nSnTHREAD_UPDATAE_YIELD_ALL = 0; pDoc->SetStatusInt(_T("Thread"), _T("nSnTHREAD_UPDATAE_YIELD_ALL"), m_nSnTHREAD_UPDATAE_YIELD_ALL);
	m_nSnTHREAD_UPDATAE_YIELD_ITS = 0; pDoc->SetStatusInt(_T("Thread"), _T("nSnTHREAD_UPDATAE_YIELD_ITS"), m_nSnTHREAD_UPDATAE_YIELD_ITS);
	...
	if (bInit) // 이어가기가 아닌경우.
	{
		...
		if (pDoc->m_pReelMapUp)
			pDoc->m_pReelMapUp->DeleteItsFilesInBuffer();
...
void CGvisR2R_PunchView::Move0(CfPoint pt, BOOL bCam, BOOL bWait)
{
	...
	else
	{
		if (!IsAuto())
		{
			if (m_pVoiceCoil[0])
			{
				dPosVoice = m_pVoiceCoil[0]->GetSmacPosition();
...
void CGvisR2R_PunchView::Move1(CfPoint pt, BOOL bCam, BOOL bWait)
{
	...
	else
	{
		if (!IsAuto())
		{
			if (m_pVoiceCoil[1])
			{
				dPosVoice = m_pVoiceCoil[1]->GetSmacPosition();
	...
	case 3:
		if (m_nMkPcs[2] < pDoc->m_Master[0].m_pPcsRgn->m_nTotPcs)	// 마킹완료Check
		{
			...
			if (!m_bCollision[0])
			{
				if (IsMoveDone0())
				{
					Move0(ptPnt, m_bCam); // (pDoc->WorkingInfo.System.bNoMk || bCam)
					m_nStepMk[2]++;
				}
			}
			...
			else if (m_bPriority[1]) // Cam1 ->
			{
				if (IsMoveDone0())
				{
					...
					Move0(ptPnt, FALSE); // (pDoc->WorkingInfo.System.bNoMk || bCam)
				}
			}
		...
		if (!IsNoMk0())
		{
			...
			else
			{
				if (m_dwStMkDn[0] + DELAY_SMAC < GetTickCount())
				{
					...
					{
						DispMain(_T("운전중"), RGB_RED);
						m_nStepMk[2] = MK_DO_REJECT; pDoc->SetStatusInt(_T("General"), _T("nStepMk[2]"), m_nStepMk[2]);
					}
					else
					{
						m_nMkPcs[2]++;
	...
	case 3:
		if (m_nMkPcs[3] < pDoc->m_Master[0].m_pPcsRgn->m_nTotPcs)	// 마킹완료Check
		{
			...
			if (!m_bCollision[1])
			{
				if (IsMoveDone1())
				{
					Move1(ptPnt, m_bCam); // (pDoc->WorkingInfo.System.bNoMk || bCam)
					m_nStepMk[3]++;
				}
			}
			else if (m_bPriority[1]) // Cam1 ->
		...
		if (!IsNoMk1())
		{
			...
			else
			{
				if (m_dwStMkDn[1] + DELAY_SMAC < GetTickCount())
				{
					...
					{
						DispMain(_T("운전중"), RGB_RED);
						m_nStepMk[3] = MK_DO_REJECT; pDoc->SetStatusInt(_T("General"), _T("nStepMk[3]"), m_nStepMk[3]);
					}
					else
					{
						m_nMkPcs[3]++;
		...
		if (m_nMkPcs[3] < pDoc->m_Master[0].m_pPcsRgn->m_nTotPcs)	// 마킹완료Check
		{
			...
			else if (m_bPriority[0]) // Cam0 <-	// 우선 순위 결정에서 Cam0에 먼저 양보함.
			{
				if (IsMoveDone1())
				{
					...
					ptPnt.x = _tstof(pDoc->WorkingInfo.Motion.sSafeZone);
					ptPnt.y = m_dEnc[AXIS_Y1];
					Move1(ptPnt, FALSE); // (pDoc->WorkingInfo.System.bNoMk || bCam)
				}
			}
			else if (m_bPriority[2]) // Cam1 <-		// 우선 순위 결정에서 Cam0에 먼저 양보함.
			{
...
void CGvisR2R_PunchView::DoMark0()
{
	...
	case MK_DONE_CHECK_START:
		...
		if (m_nMkPcs[0] < GetTotDefPcs0(nSerial))	// 마킹완료Check
		{
			...
			{
				m_nMkPcs[0]++;
				m_nStepMk[0] = MK_DONE_CHECK_END; pDoc->SetStatusInt(_T("General"), _T("nStepMk[0]"), m_nStepMk[0]);
				break;
			}
		...
		if (!IsNoMk0())
		{
			...
			else
			{
				if (m_dwStMkDn[0] + DELAY_SMAC < GetTickCount())
				{
					...
					{
						DispMain(_T("운전중"), RGB_RED);
						m_nStepMk[0] = MK_MOVE; pDoc->SetStatusInt(_T("General"), _T("nStepMk[0]"), m_nStepMk[0]);
					}
					else
					{
						m_bAnswer[2] = FALSE; pDoc->SetStatus(_T("General"), _T("bAnswer[2]"), m_bAnswer[2]);
		...
		if (m_nMkPcs[0] < GetTotDefPcs0(nSerial))
		{
			if (IsNoMk0())
			{
				if (!pDoc->WorkingInfo.System.bNoMk)
				{
					...
					{
						m_nStepMk[0] = MK_DONE_CHECK_START; pDoc->SetStatusInt(_T("General"), _T("nStepMk[0]"), m_nStepMk[0]);	// 마킹완료Check
					}
				}
				else
				{
...
void CGvisR2R_PunchView::DoMark1()
{
	...
	case MK_DONE_CHECK_START:
		...
		if (m_nMkPcs[1] < GetTotDefPcs1(nSerial))	// 마킹완료Check
		{
			...
			{
				m_nMkPcs[1]++;
				m_nStepMk[1] = MK_DONE_CHECK_END; pDoc->SetStatusInt(_T("General"), _T("nStepMk[1]"), m_nStepMk[1]);
				break;
			}
		...
		if (!IsNoMk1())
		{
			...
			else
			{
				if (m_dwStMkDn[1] + DELAY_SMAC < GetTickCount())
				{
					...
					{
						DispMain(_T("운전중"), RGB_RED);
						m_nStepMk[1] = MK_MOVE; pDoc->SetStatusInt(_T("General"), _T("nStepMk[1]"), m_nStepMk[1]);
					}
					else
					{
						m_bAnswer[3] = FALSE; pDoc->SetStatus(_T("General"), _T("bAnswer[3]"), m_bAnswer[3]);
		...
		if (m_nMkPcs[1] < GetTotDefPcs1(nSerial))
		{
			if (IsNoMk1())
			{
				if (!pDoc->WorkingInfo.System.bNoMk)
				{
					...
					{
						m_nStepMk[1] = MK_DONE_CHECK_START; pDoc->SetStatusInt(_T("General"), _T("nStepMk[1]"), m_nStepMk[1]);	// 마킹완료Check
					}
				}
				else
				{
...
BOOL CGvisR2R_PunchView::DoAutoGetLotEndSignal()
{
	...
	if (pDoc->m_pMpeSignal[19] & (0x01 << 15))
	{
		if (pDoc->WorkingInfo.LastJob.nAlignMethode == TWO_POINT)
		{
			if (pView->m_nMkStAuto > MK_ST + (Mk2PtIdx::DoneMk) && pView->m_nMkStAuto < MK_ST + (Mk2PtIdx::DoneMk) + 7)
				return m_bLotEnd; // Last Shot의 마지막 처리가 될 때까지 작업을 계속함.
		}
		else if (pDoc->WorkingInfo.LastJob.nAlignMethode == FOUR_POINT)
		{
			if (pView->m_nMkStAuto > MK_ST + (Mk4PtIdx::DoneMk) && pView->m_nMkStAuto < MK_ST + (Mk4PtIdx::DoneMk) + 7)
				return m_bLotEnd; // Last Shot의 마지막 처리가 될 때까지 작업을 계속함.
		}
...
void CGvisR2R_PunchView::DoAutoSetFdOffset()
{
	...
	BOOL bOnTestUp = pDoc->m_pMpeSignal[3] & (0x01 << 12);
	...
	BOOL bOnTestDn = pDoc->m_pMpeSignal[3] & (0x01 << 13);
	...
	BOOL bOffsetDoneUp = pDoc->m_pMpeSignal[4] & (0x01 << 13);
	...
	BOOL bOffsetDoneDn = pDoc->m_pMpeSignal[4] & (0x01 << 14);
	...
	if (pDoc->GetTestMode() == MODE_INNER || pDoc->GetTestMode() == MODE_LASER)
	{
		...
		BOOL bOffsetDoneEng = pDoc->m_pMpeSignal[4] & (0x01 << 12); // PLC 각인기 피딩 Offset Write(GUI Off) _T("MB40024C")
	...
	BOOL bOffsetDonePunch = pDoc->m_pMpeSignal[4] & (0x01 << 15); // PLC 각인기 피딩 Offset Write(GUI Off) _T("MB40024F")
...
void CGvisR2R_PunchView::Mk2PtDoMarking()
{
	...
	if ((m_bMkSt[0] || m_bMkSt[1]) && IsBuffer())
	{
		...
		case MK_ST + (Mk2PtIdx::DoMk) + 1:
			if (!m_bUpdateYield) // 펀칭이 끝나고 UpdateYield가 가능하면 (Verify에서는 UpdateYield 방지)
			{
...
BOOL CGvisR2R_PunchView::ChkCollision(int nAxisId, double dTgtPosX)
{
	...
	else if (nAxisId == AXIS_X1)
	{
		...
		if (dTgtPosX < m_dEnc[AXIS_X0] - dMg)
			return TRUE;
		if (m_bDoMk[0] && !m_bDoneMk[0])
		{
			if (!m_bDoneMk[1] && m_bTHREAD_MK[1])
			{
				if (m_dTarget[AXIS_X0] > 0.0)
				{
					if (dTgtPosX < m_dTarget[AXIS_X0] - dMg && m_dTarget[AXIS_X0] > 0.0)
						return TRUE;
					else if (m_dNextTarget[AXIS_X0] > 0.0)
					{
						if (dTgtPosX < m_dNextTarget[AXIS_X0] - dMg)
							return TRUE;
					}
				}
			}
...
void CGvisR2R_PunchView::InitPLC()
{
	...
	MpeWrite(Plc.DlgInfo.UltraSonicRunCheckTime, lData);	// (단위 [초] * 100) : 1 is 10 mSec.
}
...
void CGvisR2R_PunchView::MonDispMain() // PLC의 운전상태 표시
{
	...
	if (pDoc->m_pMpeSignal[0] & (0x01 << 6))		// 운전중(PLC가 PC에 알려주는 설비 상태) - 20141031
	{
	...
	BOOL bChkReady = pDoc->m_pMpeSignal[0] & (0x01 << 3); // PLC 운전 준비 완료
	BOOL bChkOnStopping = pDoc->m_pMpeSignal[0] & (0x01 << 0); // PLC 정지 중
	BOOL bChkAutoInit = pDoc->m_pMpeSignal[0] & (0x01 << 8); // PLC 자동 초기 모드 중
...
void CGvisR2R_PunchView::UpdateYield()
{
	...
	//{
	//	while (m_bTHREAD_UPDATAE_YIELD[0])
	//	{
	//		Sleep(30);
	//	}
	//	m_nSerialTHREAD_UPDATAE_YIELD[0] = nSerial; // 좌측 마킹한 버퍼의 시리얼
	...
	//	if (m_nBufUpSerial[1] > 0)
	//	{
	//		while (m_bTHREAD_UPDATAE_YIELD[1])
	//		{
	//			Sleep(30);
	//		}
	//		m_nSerialTHREAD_UPDATAE_YIELD[1] = m_nBufUpSerial[1]; // 우측 마킹한 버퍼의 시리얼
...
{	
	...
	BOOL bChk = pDoc->m_pMpeSignal[3] & (0x01 << 11);
	...
	bChk = pDoc->m_pMpeSignal[4] & (0x01 << 5);
	...
	bChk = pDoc->m_pMpeSignal[23] & (0x01 << 4);
	...
	bChk = pDoc->m_pMpeSignal[23] & (0x01 << 5);
...
{
	if (!pView->m_bContEngraveF)	return;
	if (m_pEngrave)
		m_pEngrave->SetMonDispMainSignal();
...
void CGvisR2R_PunchView::MakeSapp3()
{
	...
	char* pData = StringToChar(sSapp3Data);
	char* pRtn = NULL;
...
UINT CGvisR2R_PunchView::ThreadProc31(LPVOID lpContext)	// UpdateYieldDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[31].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		{
			nSerial = pThread->m_nSnTHREAD_UPDATAE_YIELD;
			pThread->UpdateYieldDn(pThread->m_nSnTHREAD_UPDATAE_YIELD);
			pThread->m_nSnTHREAD_UPDATAE_YIELD_ALL = nSerial;
...
UINT CGvisR2R_PunchView::ThreadProc32(LPVOID lpContext)	// UpdateYieldAllUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[32].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		{
			nSerial = pThread->m_nSnTHREAD_UPDATAE_YIELD_ALL;
			pThread->UpdateYieldAllUp(pThread->m_nSnTHREAD_UPDATAE_YIELD_ALL);

			if (pDoc->GetTestMode() == MODE_OUTER)
			{
				pThread->m_nSnTHREAD_UPDATAE_YIELD_ITS = nSerial;
...
UINT CGvisR2R_PunchView::ThreadProc33(LPVOID lpContext)	// UpdateYieldAllDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[33].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		{
			pThread->UpdateYieldAllDn(pThread->m_nSnTHREAD_UPDATAE_YIELD_ALL);
...
UINT CGvisR2R_PunchView::ThreadProc35(LPVOID lpContext)	// UpdateYieldInnerDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[35].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		if (pThread->m_bTHREAD_UPDATE_YIELD_INNER_DN)
		{
			...
			pThread->m_nSnTHREAD_UPDATAE_YIELD_INNER_ALL = nSerial;
...
UINT CGvisR2R_PunchView::ThreadProc36(LPVOID lpContext)	// UpdateYieldInnerAllUp()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[36].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		{
			pThread->UpdateYieldInnerAllUp(pThread->m_nSnTHREAD_UPDATAE_YIELD_INNER_ALL);
...
UINT CGvisR2R_PunchView::ThreadProc37(LPVOID lpContext)	// UpdateYieldInnerAllDn()
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pThread->m_Thread[37].GetShutdownEvent(), dwShutdownEventCheckPeriod))
	{
		...
		{
			pThread->UpdateYieldInnerAllDn(pThread->m_nSnTHREAD_UPDATAE_YIELD_INNER_ALL);
...
BOOL CGvisR2R_PunchView::WriteLastRmapInfoOnOffline(int nLastShot, int nOffline)
{
	if (!pDoc->MakeMkDir())
		return FALSE;
...
BOOL CGvisR2R_PunchView::RemakeRmapFromPcr(int nLastShot, int nOffline)// [nOffline] 0 : Not exist, 1 : Exist only Up, 2 : Exist only Dn, 3 : Exist Up and Dn
{
	...
	WriteLastRmapInfoOnOffline(nLastShot, nOffline);
...
void CGvisR2R_PunchView::InitVoiceCoil(int nCam)
{
	...
	{
		m_pVoiceCoil[nCam]->SetMarkShiftData();
		Sleep(100);
		m_pVoiceCoil[nCam]->SetMarkFinalData();
		Sleep(100);
	}
}
...
BOOL CGvisR2R_PunchView::ChkRepunching(int nCam) // 0 : Left, 1 : Right
{
	...
	if (nCam == 0)
	{
		...
		if (IDYES == nRtn)
		{
			...
			m_nStepMk[0] = MK_DONE_CHECK_START; pDoc->SetStatusInt(_T("General"), _T("nStepMk[0]"), m_nStepMk[0]);
			return TRUE; // Recheck
		}
	}
	else if (nCam == 1)
	{
		...
		if (IDYES == nRtn)
		{
			...
			m_nStepMk[1] = MK_DONE_CHECK_START; pDoc->SetStatusInt(_T("General"), _T("nStepMk[1]"), m_nStepMk[1]);
			return TRUE; // Recheck
		}
	}
...
BOOL CGvisR2R_PunchView::WriteLastRmapInfo()
{
	DWORD dwTimeOut = 30000; // 30 Sec
	DWORD dwStartTick = GetTickCount();
	BOOL bEmpty = (pDoc->m_ListSerialYield.nTot > 0) ? FALSE : TRUE;
	...
	while (!bEmpty)
	{
		...
		Sleep(100);
		bEmpty = (pDoc->m_ListSerialYield.nTot > 0) ? FALSE : TRUE;
	}

	BOOL bDualTest = pDoc->WorkingInfo.LastJob.bDualTest;

	if (pDoc->m_pReelMapUp)
		pDoc->m_pReelMapUp->WriteLastRmapInfo();

	if (bDualTest)
	{
		if (pDoc->m_pReelMapDn)
			pDoc->m_pReelMapDn->WriteLastRmapInfo();
		if (pDoc->m_pReelMapAllUp)
			pDoc->m_pReelMapAllUp->WriteLastRmapInfo();
		if (pDoc->m_pReelMapAllDn)
			pDoc->m_pReelMapAllDn->WriteLastRmapInfo();
	}

	if (pDoc->GetTestMode() == MODE_OUTER)
	{
		if (pDoc->m_pReelMapIts)
			pDoc->m_pReelMapIts->WriteLastRmapInfo();
	}
	return TRUE;
}
...
BOOL CGvisR2R_PunchView::ChkMkImgR(int nSerial, int nTotMk)
{
	...
	sDest.Format(_T("%s%s\\%s\\%s\\Punching"), pDoc->WorkingInfo.System.sPathOldFile, pDoc->WorkingInfo.LastJob.sModel,
