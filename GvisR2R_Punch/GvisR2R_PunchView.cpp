...
//#ifdef USE_MPE
//	pView->m_pMpe->Write(Plc.DlgMenu04.FeedLeadPitchAoi, lData); // 검사부 Feeding 롤러 Lead Pitch
//	pView->m_pMpe->Write(Plc.DlgMenu04.FeedLeadPitchEngrave, lData); // 각인부 Feeding 롤러 Lead Pitch
//#endif
...
void CGvisR2R_PunchView::SetMkFdPitch(double dPitch)
{
	...
	long lData = long(dPitch*1000.0);
#ifdef USE_MPE
	pView->m_pMpe->Write(Plc.DlgMenu04.FeedLeadPitchPunch, lData); // 마킹부 Feeding 롤러 Lead Pitch
#endif
...
void CGvisR2R_PunchView::SetAoiFd()
{
	...
	{
#ifdef USE_MPE
		//IoWrite(pView->Plc.DlgMenu03.FeedOnePanel, 1);	// 한판넬 이송상태 ON (PC가 ON, OFF)
		pView->m_pMpe->Write(pView->Plc.DlgMenu03.FeedOnePanel, 1);
#endif
...
void CGvisR2R_PunchView::MoveAoi(double dOffset)
{
	...
	pView->m_pMpe->Write(Plc.DlgMenu01.FeedCwAoi, 1);	// 검사부 피딩 ON (PLC가 피딩완료 후 OFF)
	if(bDualTest)
...
#ifdef USE_MPE
	long lData = (long)(dOffset * 1000.0);
	pView->m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetPunch, lData);	// 마킹부 Feeding 롤러 Offset(*1000, +:더 보냄, -덜 보냄)
	pView->m_pMpe->Write(Plc.DlgMenu01.FeedCwPunch, 1);		// 마킹부 피딩 CW ON (PLC가 피딩완료 후 OFF))
#endif
}
...
BOOL CGvisR2R_PunchView::DoAutoGetLotEndSignal()
{
	...
	if (m_bLotEnd)
	{
		...
		case LOT_END:
			...
			break;
		case LOT_END + 1:
			m_pMpe->Write(Plc.DlgMenu01.JobEnd, 1);			// 작업종료(PC가 On시키고, PLC가 확인하고 Off시킴)-20141031
	...
	if (m_bLastProc)
	{
		...
		case LAST_PROC:	// 잔량처리 1
			if (IsRun())
			{
				...
				//		m_pMpe->Write(Plc.DlgMenu01.LastJobFromAoiUp, 1);			// 잔량처리 AOI(상) 부터(PC가 On시키고, PLC가 확인하고 Off시킴)-20141112
				//		Sleep(300);
				//		m_pMpe->Write(_T("MB44012B"), 1);			// AOI 상 : PCR파일 Received
				//	}
				//	else
				...
				//		m_pMpe->Write(Plc.DlgMenu01.LastJobFromAoiDn, 1);			// 잔량처리 AOI(하) 부터(PC가 On시키고, PLC가 확인하고 Off시킴)-20141112
				//	}
				//}
				//else
				//{
				...
				//		m_pMpe->Write(Plc.DlgMenu01.LastJobFromEngrave, 1);			// 잔량처리 각인부 부터(PC가 On시키고, PLC가 확인하고 Off시킴)-20141112
				//	}
		...
		case LAST_PROC + 1:
			...
			m_pMpe->Write(pView->Plc.DlgMenu01.LastJob, 1);			// 잔량처리(PC가 On시키고, PLC가 확인하고 Off시킴)-20141031
			m_nLastProcAuto++;
			break;
		case LAST_PROC + 2:
			;
...
void CGvisR2R_PunchView::DoAutoSetFdOffset()
{
	...
	if (bDualTest) // 20160721-syd-temp
	{
		if ((m_bAoiFdWrite[0] && m_bAoiFdWrite[1]) && (!m_bAoiFdWriteF[0] && !m_bAoiFdWriteF[1]))
		{
			...
			if(bDualTest)
				pView->m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetAoiDn, (long)(-1.0*dAveX*1000.0));
			else
				pView->m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetAoiUp, (long)(-1.0*dAveX*1000.0));
			pView->m_pMpe->Write(_T("MB440111"), 0); // 검사부(상) Feeding Offset Write 완료(PC가 확인하고 Reset시킴.)-20141103
			pView->m_pMpe->Write(_T("MB440112"), 0); // 검사부(하) Feeding Offset Write 완료(PC가 확인하고 Reset시킴.)-20141103
		...
		if (m_bAoiTest[0] && !m_bAoiTest[1])
		{
			if (m_bAoiFdWrite[0] && !m_bAoiFdWriteF[0])
			{
				...
				if(bDualTest)
					pView->m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetAoiDn, (long)(-1.0*OfStUp.x*1000.0));
				else
					pView->m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetAoiUp, (long)(-1.0*OfStUp.x*1000.0));
				pView->m_pMpe->Write(_T("MB440111"), 0); // 검사부(상) Feeding Offset Write 완료(PC가 확인하고 Reset시킴.)-20141103
		...
		if (!m_bAoiTest[0] && m_bAoiTest[1])
		{
			if (m_bAoiFdWrite[1] && !m_bAoiFdWriteF[1])
			{
				...
				if(bDualTest)
					pView->m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetAoiDn, (long)(-1.0*OfStDn.x*1000.0));
				else
					pView->m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetAoiUp, (long)(-1.0*OfStDn.x*1000.0));
				pView->m_pMpe->Write(_T("MB440112"), 0); // 검사부(하) Feeding Offset Write 완료(PC가 확인하고 Reset시킴.)-20141103
	...
	else // 20160721-syd-temp
	{
		if (m_bAoiFdWrite[0] && !m_bAoiFdWriteF[0])
		{
			...
			if(bDualTest)
				pView->m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetAoiDn, (long)(-1.0*dAveX*1000.0));	// 검사부 Feeding 롤러 Offset(*1000, +:더 보냄, -덜 보냄)
			else
				pView->m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetAoiUp, (long)(-1.0*dAveX*1000.0));	// 검사부 Feeding 롤러 Offset(*1000, +:더 보냄, -덜 보냄)
			pView->m_pMpe->Write(_T("MB440111"), 0); // 검사부(상) Feeding Offset Write 완료(PC가 확인하고 Reset시킴.)-20141103
...
void CGvisR2R_PunchView::InitPLC()
{
	...
	m_pMpe->Write(Plc.DlgInfo.TotalReelLength, lData);	// 전체 Reel 길이 (단위 M * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Lot.sSeparateDist) * 1000.0);
	m_pMpe->Write(Plc.DlgInfo.LotLength, lData);	// Lot 분리 길이 (단위 M * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Lot.sCuttingDist) * 1000.0);
	m_pMpe->Write(Plc.DlgInfo.LotCutLength, lData);	// Lot 분리 후 절단위치 (단위 M * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Lot.sStopDist) * 1000.0);
	m_pMpe->Write(Plc.DlgInfo.StopLength, lData);	// 일시정지 길이 (단위 M * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sAoiTq) * 1000.0);
	m_pMpe->Write(Plc.DlgMenu04.FeedTorqueAoi, lData);	// 검사부 Tension 모터 토크값 (단위 Kgf * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sMkTq) * 1000.0);
	m_pMpe->Write(Plc.DlgMenu04.FeedTorquePunch, lData);	// 마킹부 Tension 모터 토크값 (단위 Kgf * 1000) _T("ML45044")
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sEngraveTq) * 1000.0);
	m_pMpe->Write(Plc.DlgMenu04.FeedTorqueEngrave, lData);	// 각인부 Tension 모터 토크값 (단위 Kgf * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.LastJob.sOnePnlLen) * 1000.0);
	m_pMpe->Write(Plc.DlgInfo.OneShotLength, lData);	// 한 판넬 길이 (단위 mm * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sMkFdVel) * 1000.0);
	m_pMpe->Write(Plc.DlgMenu04.FeedSpeed, lData);	// 한 판넬 Feeding 속도 (단위 mm/sec * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sMkJogVel) * 1000.0);
	m_pMpe->Write(Plc.DlgMenu04.FeedOnSpeed, lData);	// 연속공급 속도 (단위 mm/sec * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sAoiJogAcc) * 1000.0);
	m_pMpe->Write(Plc.DlgMenu04.FeedOnAcc, lData);	// 연속공급 가속도 (단위 mm/s^2 * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sMkFdAcc) * 1000.0);
	m_pMpe->Write(Plc.DlgMenu04.FeedAcc, lData);	// 한 판넬 Feeding 가속도 (단위 mm/s^2 * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sAoiFdLead) * 1000.0);
	m_pMpe->Write(Plc.DlgMenu04.FeedLeadPitchAoi, lData);	// 검사부 Feeding 롤러 Lead Pitch (단위 mm * 1000)
	m_pMpe->Write(Plc.DlgMenu04.FeedLeadPitchEngrave, lData);	// 각인부 Feeding 롤러 Lead Pitch (단위 mm * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sMkFdLead) * 1000.0);
	m_pMpe->Write(Plc.DlgMenu04.FeedLeadPitchPunch, lData);	// 마킹부 Feeding 롤러 Lead Pitch (단위 mm * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sStBufPos) * 1000.0);
	pView->m_pMpe->Write(Plc.DlgMenu04.BufferPosStAoi, lData);	// 버퍼 관련 설정 롤러 초기위치(단위 mm * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sFdMkAoiInitDist) * 1000.0);
	pView->m_pMpe->Write(Plc.DlgMenu04.FeedLengthFromAoiToPunch, lData);	// AOI(하)에서 마킹까지 거리 (단위 mm * 1000)
	lData = (long)(_tstoi(pDoc->WorkingInfo.Motion.sFdAoiAoiDistShot) * 1000);
	pView->m_pMpe->Write(Plc.DlgMenu04.ShotNumFromAoiUpToAoiDn, lData);	// AOI(상)에서 AOI(하) Shot수 (단위 Shot수 * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sFdEngraveAoiInitDist) * 1000.0);
	pView->m_pMpe->Write(Plc.DlgMenu04.FeedLengthFromEngraveToAoi, lData);	// 각인부에서 AOI(상)까지 거리 (단위 mm * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sFdBarcodeOffsetVel) * 1000.0);
	m_pMpe->Write(Plc.DlgMenu04.FeedSpeed2dCode, lData);	// 2D 바코드 리더기위치까지 Feeding 속도 (단위 mm/sec * 1000)
	lData = (long)(_tstof(pDoc->WorkingInfo.Motion.sFdBarcodeOffsetAcc) * 1000.0);
	m_pMpe->Write(Plc.DlgMenu04.FeedAcc2dCode, lData);	// 2D 바코드 리더기위치까지 Feeding 가속도 (단위 mm/s^2 * 1000)

	lData = (long)(_tstof(pDoc->WorkingInfo.LastJob.sUltraSonicCleannerStTim) * 100.0);
	m_pMpe->Write(Plc.DlgInfo.UltraSonicStTimeAoiDn, lData);	// AOI_Dn (단위 [초] * 100) : 1 is 10 mSec.
	m_pMpe->Write(Plc.DlgInfo.UltraSonicStTimeAoiUp, lData);	// AOI_Up (단위 [초] * 100) : 1 is 10 mSec.
	
	m_pMpe->Write(Plc.DlgInfo.Use380mm, pDoc->WorkingInfo.LastJob.bUse380mm ? 1 : 0);	// EPC실린더(제품소->OFF/제품대->ON)
#endif
}
...
void CGvisR2R_PunchView::SetPlcParam()
{
	...
	m_pMpe->Write(Plc.DlgInfo.StopLength, long(_tstof(pDoc->WorkingInfo.LastJob.sTempPauseLen)*1000.0));
	//IoWrite(Plc.DlgInfo.LotLength, long(_tstof(pDoc->WorkingInfo.LastJob.sLotSepLen)*1000.0));	// Lot 분리 길이 (단위 M * 1000)
	m_pMpe->Write(Plc.DlgInfo.LotLength, long(_tstof(pDoc->WorkingInfo.LastJob.sLotSepLen)*1000.0));
	//IoWrite(Plc.DlgInfo.LotCutLength, long(_tstof(pDoc->WorkingInfo.LastJob.sLotCutPosLen)*1000.0));	// Lot 분리 후 절단위치 (단위 M * 1000)
	m_pMpe->Write(Plc.DlgInfo.LotCutLength, long(_tstof(pDoc->WorkingInfo.LastJob.sLotCutPosLen)*1000.0));
	m_pMpe->Write(Plc.DlgInfo.SampleTestShotNum, (long)_tstoi(pDoc->WorkingInfo.LastJob.sSampleTestShotNum));	// 샘플검사 Shot수

	if (pDoc->WorkingInfo.LastJob.bTempPause)
	{
		//pView->IoWrite(Plc.DlgInfo.UseStopLength, 1);	// 일시정지사용(PC가 On시키고, PLC가 확인하고 Off시킴)-20141031
		m_pMpe->Write(Plc.DlgInfo.UseStopLength, 1);
		ChkTempStop(TRUE);
	}
	else
	{
		//IoWrite(Plc.DlgInfo.UseStopLength, 0);	// 일시정지사용(PC가 On시키고, PLC가 확인하고 Off시킴)-20141031
		m_pMpe->Write(Plc.DlgInfo.UseStopLength, 0);
		ChkTempStop(FALSE);
	}
	...
	{
		SetLotLastShot();
		m_pMpe->Write(Plc.DlgInfo.UseLotLength, 1);
		//IoWrite(Plc.DlgInfo.UseLotLength, 1);	// 로트분리사용(PC가 On시키고, PC가 확인하고 Off시킴)-20141031
	}
	else
		m_pMpe->Write(Plc.DlgInfo.UseLotLength, 0);
	//IoWrite(Plc.DlgInfo.UseLotLength, 0);	// 로트분리사용(PC가 On시키고, PC가 확인하고 Off시킴)-20141031

	m_pMpe->Write(pView->Plc.DlgInfo.SingleMode, (pDoc->WorkingInfo.LastJob.bDualTest) ? 0 : 1);		// 단면 검사 On
	m_pMpe->Write(Plc.DlgInfo.SampleTest, (pDoc->WorkingInfo.LastJob.bSampleTest) ? 1 : 0);		// Sample 검사 On
	m_pMpe->Write(Plc.DlgInfo.OneMetal, (pDoc->WorkingInfo.LastJob.bOneMetal) ? 1 : 0);		// One Metal On
	m_pMpe->Write(Plc.DlgInfo.TwoMetal, (pDoc->WorkingInfo.LastJob.bTwoMetal) ? 1 : 0);		// Two Metal On
	m_pMpe->Write(pView->Plc.DlgMenu03.UseCore150mmRecoiler, (pDoc->WorkingInfo.LastJob.bCore150Recoiler) ? 1 : 0);	// Recoiler Core 150mm On
	m_pMpe->Write(pView->Plc.DlgMenu03.UseCore150mmUncoiler, (pDoc->WorkingInfo.LastJob.bCore150Uncoiler) ? 1 : 0);	// Uncoiler Core 150mm On

	m_pMpe->Write(pView->Plc.DlgInfo.CleanRollerAoiUp), (pDoc->WorkingInfo.LastJob.bUseAoiUpCleanRoler ? 1 : 0));
	m_pMpe->Write(pView->Plc.DlgInfo.CleanRollerAoiDn), (pDoc->WorkingInfo.LastJob.bUseAoiDnCleanRoler ? 1 : 0));
#endif
}
...
void CGvisR2R_PunchView::InitIoWrite()
{
	...
	if(bDualTest)
		m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetAoiDn, 0);
	else
		m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetAoiUp, 0);
	//IoWrite(Plc.DlgFrameHigh.FeedOffsetPunch, 0); // 마킹부 Feeding 롤러 Offset(*1000, +:더 보냄, -덜 보냄, PC가 쓰고 PLC에서 지움)
	m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetPunch, 0);
...
#ifdef USE_MPE
			pView->m_pMpe->Write(Plc.DlgInfo.UseLotLength, 0);	// 로트분리사용(PC가 On시키고, PC가 확인하고 Off시킴)-20141031
			//pView->IoWrite(Plc.DlgInfo.UseLotLength, 1);
#endif
			if (pDoc->m_pReelMap)
...
#ifdef USE_MPE
	if (pView->m_pMpe)
		pView->m_pMpe->Write(pView->Plc.DlgInfo.SingleMode, bOn ? 0 : 1);		// 단면 검사 On
#endif
	if (m_pDlgFrameHigh)
...
void CGvisR2R_PunchView::SetTwoMetal(BOOL bSel, BOOL bOn)
{
	if (bSel)
	{
		if (bOn)
		{
			...
			m_pMpe->Write(Plc.DlgInfo.TwoMetal, 1);
		...
		else
		{
			...
			m_pMpe->Write(Plc.DlgInfo.TwoMetal, 0);
...
//#endif
	...
	else
	{
		if (bOn)
		{
			...
			m_pMpe->Write(Plc.DlgInfo.OneMetal, 1);
		...
		else
		{
			...
			m_pMpe->Write(Plc.DlgInfo.OneMetal, 0);
...
#ifdef USE_MPE
	long lData = (long)(dOffset * 1000.0);
	pView->m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetEngrave, lData);		// 각인부 Feeding 롤러 Offset(*1000, +:더 보냄, -덜 보냄, PC가 쓰고 PLC에서 지움)
	pView->m_pMpe->Write(Plc.DlgMenu01.FeedCwEngrave, 1);		// 각인부 피딩 CW ON (PLC가 피딩완료 후 OFF)
#endif
}
...
void CGvisR2R_PunchView::DoAutoSetFdOffsetEngrave()
{
	...
	if (m_bEngFdWrite && !m_bEngFdWriteF)
	{
		...
		{
			pDoc->LogAuto(_T("PC: 각인부 Feeding Offset Write 완료(PC가 확인하고 Reset시킴.)"));
			m_pMpe->Write(Plc.DlgFrameHigh.FeedOffsetEngrave, (long)(dAveX*1000.0));	// 각인부 Feeding 롤러 Offset(*1000, +:더 보냄, -덜 보냄, PC가 쓰고 PLC에서 지움)
			m_pMpe->Write(_T("MB44011A"), 0);					// 각인부 Feeding Offset Write 완료(PC가 확인하고 Reset시킴.)
		}
		Sleep(10);
#endif
...
void CGvisR2R_PunchView::SetEngFd()
{
	...
	{
#ifdef USE_MPE
		pView->m_pMpe->Write(pView->Plc.DlgMenu03.FeedOnePanel, 1);	// 한판넬 이송상태 ON (PC가 ON, OFF)
#endif
		CString sData, sPath = PATH_WORKING_INFO;
	...
	for (i = 0; i < _SigInx::_EndIdx; i++)
	{
		if (m_bRcvSig[i])
		{
			...
			case _SigInx::_CcwModRe:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.OneMetal, (long)(pDoc->BtnStatus.Induct.Rc ? 1 : 0)); // 마킹(GUI) 리코일러 정방향(Off->CW/On->CCW)
				break;
			case _SigInx::_CcwModUn:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.TwoMetal, (long)(pDoc->BtnStatus.Induct.Uc ? 1 : 0)); // 마킹(GUI) 언코일러 정방향(Off->CW/On->CCW)
				break;
			//case _SigInx::_Core150Re:
			//	if (m_pMpe)
			//		m_pMpe->Write(pView->Plc.DlgMenu03.UseCore150mmRecoiler, (long)(pDoc->BtnStatus.Core150.Rc ? 1 : 0)); // Marking GUI Recoiler Core 150[mm](PC On/Off)
			//	break;
			//case _SigInx::_Core150Un:
			//	if (m_pMpe)
			//		m_pMpe->Write(pView->Plc.DlgMenu03.UseCore150mmUncoiler, (long)(pDoc->BtnStatus.Core150.Uc ? 1 : 0)); // Marking GUI Uncoiler Core 150[mm](PC On/Off)
			//	break;
			...
			case _SigInx::_OnePnlMk:
				if (m_pMpe)
					m_pMpe->Write(pView->Plc.DlgMenu03.FeedOnePanel, (long)(pDoc->BtnStatus.Mk.MvOne ? 1: 0));
				break;
			...
			case _SigInx::_OnePnlAoiDn:
				if (m_pMpe)
					m_pMpe->Write(pView->Plc.DlgMenu03.FeedOnePanel, (long)(pDoc->BtnStatus.AoiDn.MvOne ? 1 : 0)); // 한판넬 이송상태 ON (PC가 ON, OFF)
				break;
			...
			case _SigInx::_OnePnlAoiUp:
				if (m_pMpe)
					m_pMpe->Write(pView->Plc.DlgMenu03.FeedOnePanel, (long)(pDoc->BtnStatus.AoiUp.MvOne ? 1 : 0)); // 한판넬 이송상태 ON (PC가 ON, OFF)
				break;
			case _SigInx::_OnePnlEng:
				if (m_pMpe)
					m_pMpe->Write(pView->Plc.DlgMenu03.FeedOnePanel, (long)(pDoc->BtnStatus.Eng.MvOne ? 1 : 0)); // 한판넬 이송상태 ON (PC가 ON, OFF)
				break;
			...
			case _SigInx::_RecoilerCcw:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.OneMetal, (long)(pDoc->BtnStatus.Induct.Rc ? 1 : 0));
			...
			case _SigInx::_UncoilerCcw:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.TwoMetal, (long)(pDoc->BtnStatus.Induct.Uc ? 1 : 0));
			...
			case _SigInx::_DoorRecoiler:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.DoorSensorRecoiler, (long)(pDoc->WorkingInfo.LastJob.bRclDrSen ? 1 : 0));	// 리코일러Door센서 사용
				break;
			case _SigInx::_DoorAoiUp:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.DoorSensorAoiUp, (long)(pDoc->WorkingInfo.LastJob.bAoiUpDrSen ? 1 : 0));	// AOI(상) Door센서 사용
				break;
			case _SigInx::_DoorAoiDn:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.DoorSensorAoiDn, (long)(pDoc->WorkingInfo.LastJob.bAoiDnDrSen ? 1 : 0));	// AOI(하) Door센서 사용
				break;
			case _SigInx::_DoorMk:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.DoorSensorPunch, (long)(pDoc->WorkingInfo.LastJob.bMkDrSen ? 1 : 0));	// 마킹Door센서 사용
				break;
			case _SigInx::_DoorEngrave:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.DoorSensorEngrave, (long)(pDoc->WorkingInfo.LastJob.bEngvDrSen ? 1 : 0));	// 각인부 Door센서 사용
				break;
			case _SigInx::_DoorUncoiler:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.DoorSensorUncoiler, (long)(pDoc->WorkingInfo.LastJob.bUclDrSen ? 1 : 0));	// 언코일러Door센서 사용
				break;
			case _SigInx::_CleannerAoiUp:
				if (m_pMpe)
					m_pMpe->Write(pView->Plc.DlgInfo.CleanRollerAoiUp, (long)(pDoc->WorkingInfo.LastJob.bUseAoiUpCleanRoler ? 1 : 0));
				break;
			case _SigInx::_CleannerAoiDn:
				if (m_pMpe)
					m_pMpe->Write(pView->Plc.DlgInfo.CleanRollerAoiDn, (long)(pDoc->WorkingInfo.LastJob.bUseAoiDnCleanRoler ? 1 : 0));
				break;
			case _SigInx::_UltraSonicAoiDn:
				if (m_pMpe)
					m_pMpe->Write(pView->Plc.DlgInfo.UltraSonicAoi, (long)(pDoc->WorkingInfo.LastJob.bUseAoiDnUltrasonic ? 1 : 0));
				break;
			case _SigInx::_UltraSonicEngrave:
				if (m_pMpe)
					m_pMpe->Write(pView->Plc.DlgInfo.UltraSonicEngrave, (long)(pDoc->WorkingInfo.LastJob.bUseEngraveUltrasonic ? 1 : 0));
				break;
			case _SigInx::_Use380mm:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.Use380mm, (long)(pDoc->WorkingInfo.LastJob.bUse380mm ? 1 : 0));	// EPC실린더(제품소->OFF/제품대->ON)
				break;
			case _SigInx::_TempPause:
				if (m_pMpe)
					m_pMpe->Write(Plc.DlgInfo.UseStopLength, (long)(pDoc->WorkingInfo.LastJob.bTempPause ? 1 : 0));	// 일시정지사용(PC가 On시키고, PLC가 확인하고 Off시킴)
