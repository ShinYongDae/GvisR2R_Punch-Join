...
BOOL CGvisR2R_PunchDoc::GetAoiInfoUp(int nSerial, int *pNewLot, BOOL bFromBuf) // TRUE: CHANGED, FALSE: NO CHANGED 
{
	...
	*pNewLot = 0;
	...
	{
		bUpdate = TRUE;
		if (WorkingInfo.LastJob.sModel != Status.PcrShare[0].sModel)
		{
			WorkingInfo.LastJob.sModel = Status.PcrShare[0].sModel;
			sMsg.Format(_T("Model Change %s"), Status.PcrShare[0].sModel);
			pDoc->LogAuto(sMsg);
		}

		if (WorkingInfo.LastJob.sLayerUp != Status.PcrShare[0].sLayer)
		{
			WorkingInfo.LastJob.sLayerUp = Status.PcrShare[0].sLayer;
			sMsg.Format(_T("LayerUp Change %s"), Status.PcrShare[0].sLayer);
			pDoc->LogAuto(sMsg);
		}
		*pNewLot = 1;
...
BOOL CGvisR2R_PunchDoc::GetAoiInfoDn(int nSerial, int *pNewLot, BOOL bFromBuf) // TRUE: CHANGED, FALSE: NO CHANGED 
{
	*pNewLot = 0;
	...
	{
		bUpdate = TRUE;
		if (WorkingInfo.LastJob.sModel != Status.PcrShare[1].sModel)
		{
			WorkingInfo.LastJob.sModel = Status.PcrShare[1].sModel;
			sMsg.Format(_T("Model Change %s"), Status.PcrShare[1].sModel);
			pDoc->LogAuto(sMsg);
		}
		if (WorkingInfo.LastJob.sLayerDn != Status.PcrShare[1].sLayer)
		{
			WorkingInfo.LastJob.sLayerDn = Status.PcrShare[1].sLayer;
			sMsg.Format(_T("LayerDn Change %s"), Status.PcrShare[1].sLayer);
			pDoc->LogAuto(sMsg);
		}
		*pNewLot = 1;
	}
...
int CGvisR2R_PunchDoc::LoadPCRDn(int nSerial, BOOL bFromShare)	// return : 2(Failed), 1(정상), -1(Align Error, 노광불량), -2(Lot End)
{
	...
	if (nTotDef > 0)
	{
		for (i = 0; i < nTotDef; i++)
		{
			...
			//pDoc->m_Master[1].m_pPcsRgn->GetMkMatrix(pDoc->m_Master[1].MasterInfo.nActionCode, Rotate180(m_pPcr[1][nIdx]->m_pDefPcs[i]), nC, nR);
			pDoc->m_Master[1].m_pPcsRgn->GetMkMatrix(pDoc->m_Master[1].MasterInfo.nActionCode, pDoc->m_pPcr[1][nIdx]->m_pDefPcs[i], nC, nR);
			m_pPcr[1][nIdx]->m_arDefType[nR][nC] = m_pPcr[1][nIdx]->m_pDefType[i];
			m_pPcr[1][nIdx]->m_arPcrLineNum[nR][nC] = i;
			//pDoc->m_Master[1].m_pPcsRgn->GetMkMatrix(pDoc->m_Master[1].MasterInfo.nActionCode, m_pPcr[1][nIdx]->m_pDefPcs[i], nC, nR);
